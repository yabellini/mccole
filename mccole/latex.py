...
def doc_to_latex(doc, config=None):
    """Convert Document to LaTeX using information in `config` (if any)."""
    with McColeLaTeX(config) as renderer:
        patch_divs(doc)
        return renderer.render(doc)


def md_to_latex(text, config=None):
    """Convert Markdown to LaTeX using information in `config` (if any)."""
    with McColeLaTeX(config) as renderer:
        doc = Document(text)
        patch_divs(doc)
        return renderer.render(doc)
...
class McColeLaTeX(LaTeXRenderer):
    ...
    def render_bib_cite(self, token):
        ...
        cites = ",".join(token.cites)
        return f"\\cite{{{cites}}}"

    def render_fig_def(self, token):
        """Render figure definition."""
        fields = [token.label, token.file, token.alt, token.cap]
        fields = [f"{{{f.strip()}}}" for f in fields]
        return f"\\fig{fields}"
    ...
    def render_gloss_index_ref(self, token):
        ...
        return f"\\giref{{{gloss_ref}}}{{{index_ref}}}{{{text}}}"
    ...
    def render_tbl_def(self, token):
        ...
        fields = [token.label, token.file, token.cap]
        fields = [f"{{{f.strip()}}}" for f in fields]
        return f"\\tbl{fields}"
